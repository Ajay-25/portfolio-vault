...
import { getAllInsurancePolicies } from "@/lib/data/portfolio";
...
export async function InsuranceOverviewContent() {
  ...
  const byPerson = [
    {
      label: WEALTH_OWNERS.mine.label,
      type: "primary",
      policies: allPolicies.filter((p) => p.portfolio.type === "primary"),
    },
    {
      label: WEALTH_OWNERS.mother.label,
      type: "secondary",
      policies: allPolicies.filter((p) => p.portfolio.type === "secondary"),
    },
  ];
